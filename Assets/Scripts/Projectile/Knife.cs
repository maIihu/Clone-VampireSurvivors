...
public class Knife : Projectile
{
    ...
    public Knife(GameObject projectilePrefab, Transform attackPoint, float attackCooldown = 1f, 
        float projectileSpeed = 8f, int projectileCount = 1, int projectileDamage = 6, bool isActive = false) 
    ...
    public override void Throw()
    {
        if (IsActive && Time.time >= NextAttackTime)
        {
            for (int i = 0; i < ProjectileCount; i++)
            {
                ...
                Vector2 direction = (nearestEnemy.position - AttackPoint.position).normalized;
